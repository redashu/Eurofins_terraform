...
resource "azurerm_public_ip" "ashu-public-ip1" {
    name = "${var.my_publicIP}-${count.index}" 
    resource_group_name = azurerm_resource_group.name.name
    location = var.region_name
...
resource "azurerm_network_interface" "ashu-nic1" {
  name = "${var.my_net_interface}-nic${count.index}" #ashu_network_interface-nic0 , ashu_network_interface-nic1
  resource_group_name = azurerm_resource_group.name.name
  location = var.region_name
  count = 4
  ...
  ip_configuration {
    ...
    public_ip_address_id = azurerm_public_ip.ashu-public-ip1[count.index].id
    name = "ashu-ip1-${count.index}"
  }
